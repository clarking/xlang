...
namespace xlang {
	...
	typedef enum {
		PRIMARY_EXPR, ASSGN_EXPR, SIZEOF_EXPR, CAST_EXPR, ID_EXPR, FUNC_CALL_EXPR,
	} expr_t;
	...
	struct primary_expr_t {
		...
		primary_expr_t *left;
		primary_expr_t *right;
		//unary node of parse tree
		//if operator is unary
		primary_expr_t *unary_node;
	...
	struct cast_expr_t {
		...
		id_expr_t *target; //identifier expression that need to be cast
		void print();
	};
	...
	struct call_expr_t {
		id_expr_t *function;                  //function name(could be simple or from record member(e.g x.y->func()))
	...
	*/
	...
	typedef enum {
		WHILE_STMT, DOWHILE_STMT, FOR_STMT
	} iter_stmt_t;
	...
	*/
	...
	typedef enum {
		BREAK_JMP, CONTINUE_JMP, RETURN_JMP, GOTO_JMP
	} jmp_stmt_t;
	...
	*/
	...
	//statement types
	typedef enum {
		LABEL_STMT, EXPR_STMT, SELECT_STMT, ITER_STMT, JUMP_STMT, DECL_STMT, ASM_STMT
	} stmt_t;
	...
	*/
	...
	class tree {
		...
		static sizeof_expr_t *get_sizeof_expr_mem();
		
		static void delete_sizeof_expr(sizeof_expr_t **);
		
		static cast_expr_t *get_cast_expr_mem();
		
		static void delete_cast_expr(cast_expr_t **);
		
		static primary_expr_t *get_primary_expr_mem();
		
		static void delete_primary_expr(primary_expr_t **);
		
		static id_expr_t *get_id_expr_mem();
		
		static void delete_id_expr(id_expr_t **);
		
		static expr *get_expr_mem();
		
		static void delete_expr(expr **);
		
		static assgn_expr_t *get_assgn_expr_mem();
		
		static void delete_assgn_expr(assgn_expr_t **);
		
		static call_expr_t *get_func_call_expr_mem();
		
		static void delete_func_call_expr(call_expr_t **);
		...
		static void get_inorder_primary_expr(primary_expr_t **);
		...
		private:
		static std::stack<primary_expr_t *> pexpr_stack;
	};
}
