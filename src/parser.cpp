...
namespace xlang {
	...
	xlang::st_node *global_symtab;
	//global record table
	xlang::st_record_symtab *record_table;
...
bool xlang::parser::expect_token(token_t tk) {
	...
	if (tok.number != tk) {
		...
		if (find_it != token_lexeme_table.end()) {
			...
			log_error_at(xlang::filename, loc, "expected ", find_it->second);
...
bool xlang::parser::expect_token(token_t tk, bool consume_token) {
	...
	if (tok.number != tk) {
		...
		if (find_it != token_lexeme_table.end()) {
			...
			log_error_at(xlang::filename, loc, "expected ", find_it->second,
			             " but found " + s_quotestring(tok.string));
			std::cout << expr_list << std::endl;
...
bool xlang::parser::expect_token(token_t tk, bool consume_token, std::string str) {
	...
	if (tok.number != tk) {
		log_error_at(xlang::filename, tok.loc, "expected ", str);
		std::cout << expr_list << std::endl;
...
bool xlang::parser::expect_token(token_t tk, bool consume_token,
	...
	if (tok.number != tk) {
		log_error_at(xlang::filename, tok.loc, "expected ", str, arg);
		std::cout << expr_list << std::endl;
...
bool xlang::parser::expect_token(const char *format...) {
	...
	log_error_at(xlang::filename, tok.loc, "expected ");
...
void xlang::parser::consume_tokens_till(terminator_t &terminator) {
	...
	while ((tok = lex->get_next_token()).number != END_OF_FILE) {
...
bool xlang::parser::peek_type_specifier(std::vector<token> &tokens) {
	...
	if (tok.number == KEY_VOID || tok.number == KEY_CHAR
	    || tok.number == KEY_DOUBLE || tok.number == KEY_FLOAT
	    || tok.number == KEY_INT || tok.number == KEY_SHORT
	    || tok.number == KEY_LONG || tok.number == IDENTIFIER) {
...
void xlang::parser::primary_expression(terminator_t &terminator) {
	...
	if (match_with_terminator(terminator, tok.number)) {
	...
	switch (tok.number) {
		case PARENTH_OPEN : {
			...
			if (peek_token(PARENTH_CLOSE)) {
				...
				log_error_at(xlang::filename, tok2.loc, "expression expected ", tok2.string);
				return;
			}
			...
			if (parenth_stack.size() > 0 && expect_token(PARENTH_CLOSE)) {
				if (!check_parenth()) {
					log_error(xlang::filename, "unbalanced parenthesis");
				...
				if (peek_binary_operator() || peek_unary_operator()) {
					...
					token tok2 = lex->get_next_token();
					if (!check_parenth()) {
						log_error_at(xlang::filename, tok2.loc, "unbalanced parenthesis ", tok2.string);
					}
					else {
						expr_list.push_back(tok2);
						primary_expression(terminator);
					...
					if (!is_expr_terminator_consumed || !is_expr_terminator_got) {
						log_error_at(xlang::filename, tok.loc, get_terminator_string(terminator) + "expected");
					}
					if (check_parenth()) {
						log_error(xlang::filename, "unbalanced parenthesis");
						...
						log_error(xlang::filename,
						          get_terminator_string(terminator) + "expected but found " + tok.string);
					}
				}
			}
		}
		...
		case LIT_CHAR : {
			...
			if (peek_binary_operator() || peek_unary_operator()) {
				...
				if (peek_token(PARENTH_OPEN) || peek_token(IDENTIFIER)) {
					...
					token tok2 = lex->get_next_token();
					log_error_at(xlang::filename, tok2.loc, "literal or expression expected ", tok2.string);
					for (const auto &e: expr_list) {
						log_error(e.number, e.string, "\n");
					}
					std::cout << std::endl;
					return;
				}
			...
			if (peek_token(terminator)) {
				...
				if (peek_token(PARENTH_CLOSE)) {
					if (parenth_stack.size() == 0) {
						token tok2 = lex->get_next_token();
						log_error_at(xlang::filename, tok2.loc, "error ", tok2.string);
					}
				}
					...
					if (!is_expr_terminator_consumed) {
						log_error_at(xlang::filename, tok2.loc, get_terminator_string(terminator) + "expected");
						return;
					}
				}
				else if (peek_expr_literal_token()) {
		...
		case BIT_COMPL : {
			...
			if (unary_operator(tok.number)) {
				...
				if (peek_token(PARENTH_OPEN) || peek_expr_literal_token() || peek_token(IDENTIFIER)) {
					...
					token tok2 = lex->get_next_token();
					log_error_at(xlang::filename, tok2.loc, "literal expected ", tok2.string);
					return;
				}
			}
		}
		...
		default : {
			log_error_at(xlang::filename, tok.loc, "primaryexpr invalid token ", tok.string);
			return;
		}
			break;
	}
		...
		switch ((*expr_it).number) {
			...
			case ADDROF_OP :
				if (post_stack.empty() || post_stack.top().number == PARENTH_OPEN) {
					...
					if (!post_stack.empty() &&
					    (operator_precedence((*expr_it).number) >
					     operator_precedence(post_stack.top().number))) {
			...
			case PARENTH_CLOSE :
				...
				if (!post_stack.empty() &&
				    post_stack.top().number == PARENTH_OPEN)
					post_stack.pop();
				break;
			...
			default :
				log_error_at(xlang::filename, (*expr_it).loc, "error in conversion into postfix expression ", (*expr_it).string);
				return;
		}
...
xlang::primary_expr *xlang::parser::get_primary_expr_tree() {
	std::stack<primary_expr *> extree_stack;
	std::list<token>::iterator post_it;
	primary_expr *expr = nullptr, *oprtr = nullptr;
	...
	for (post_it = postfix_expr.begin(); post_it != postfix_expr.end(); post_it++) {
		...
		else if (binary_operator((*post_it).number) || (*post_it).number == DOT_OP
		         || (*post_it).number == ARROW_OP) {
...
xlang::id_expr *xlang::parser::get_id_expr_tree() {
	std::stack<id_expr *> extree_stack;
	std::list<token>::iterator post_it;
	std::list<token> postfix_expr;
	id_expr *expr = nullptr, *oprtr = nullptr;
	id_expr *temp = nullptr;
	...
	for (post_it = postfix_expr.begin(); post_it != postfix_expr.end(); post_it++) {
		...
		else if ((*post_it).number == INCR_OP || (*post_it).number == DECR_OP
		         || (*post_it).number == ADDROF_OP) {
			oprtr = xlang::tree::get_id_expr_mem();
			oprtr->tok = *post_it;
			oprtr->is_id = false;
			oprtr->is_oprtr = true;
...
void xlang::parser::id_expression(terminator_t &terminator) {
	...
	if (tok.number == IDENTIFIER) {
		...
		if (peek_token(terminator)) {
			...
			log_error_at(xlang::filename, tok.loc, st + " expected in id expression but found ", tok.string);
			std::cout << "    " << expr_list << std::endl;
		...
		log_error_at(xlang::filename, tok.loc, " identifier expected but found ", tok.string);
		std::cout << "    " << expr_list << std::endl;
	...
	if (expect_token(SQUARE_OPEN_BRACKET)) {
		...
		if (peek_constant_expression() || peek_identifier()) {
			...
			if (peek_token(SQUARE_OPEN_BRACKET)) {
				...
				log_error(xlang::filename, "; , ) expected ");
				std::cout << expr_list << std::endl;
				return;
			}
		}
			...
			token tok2 = lex->get_next_token();
			log_error(xlang::filename, "constant expression expected ", tok2.string);
			std::cout << expr_list << std::endl;
...
void xlang::parser::pointer_operator_sequence() {
	...
	while ((tok = lex->get_next_token()).number == ARTHM_MUL) {
		tok.number = PTR_OP;
		expr_list.push_back(tok);
	}
	lex->unget_token(tok);
...
void xlang::parser::pointer_indirection_access(terminator_t &terminator) {
	...
	if (peek_token(IDENTIFIER)) {
		...
		log_error(xlang::filename, "identifier expected in pointer indirection");
		std::cout << expr_list << std::endl;
...
*/
xlang::id_expr *xlang::parser::prefix_incr_expression(terminator_t &terminator) {
	id_expr *pridexpr = nullptr;
	...
	if (peek_token(IDENTIFIER)) {
		...
		log_error(xlang::filename, "identifier expected ");
		std::cout << expr_list << std::endl;
...
*/
xlang::id_expr *xlang::parser::prefix_decr_expression(terminator_t &terminator) {
	id_expr *pridexpr = nullptr;
...
void xlang::parser::postfix_incr_expression(terminator_t &terminator) {
	...
	if (peek_token(terminator)) {
		...
		tok = lex->get_next_token();
		log_error_at(xlang::filename, tok.loc, "; , ) expected but found " + tok.string);
		std::cout << expr_list << std::endl;
...
address-of-expression :
  & id-expression
*/
xlang::id_expr *xlang::parser::address_of_expression(terminator_t &terminator) {
	id_expr *addrexpr = nullptr;
...
sizeof-expression :
  ...
  sizeof ( identifier )
*/
xlang::sizeof_expr *xlang::parser::sizeof_expression(terminator_t &terminator) {
	sizeof_expr *sizeofexpr = xlang::tree::get_sizeof_expr_mem();
	...
	if (peek_type_specifier(simple_types)) {
		...
		log_error(xlang::filename, "simple types, class names or identifier expected for sizeof ");
		terminator2.clear();
		terminator2.push_back(PARENTH_CLOSE);
...
*/
xlang::cast_expr *xlang::parser::cast_expression(terminator_t &terminator) {
	cast_expr *cstexpr = xlang::tree::get_cast_expr_mem();
	token tok;
	...
	if (peek_token(IDENTIFIER)) {
		...
		tok = lex->get_next_token();
		log_error_at(xlang::filename, tok.loc, " identifier expected in cast expression");
...
void xlang::parser::cast_type_specifier(cast_expr **expr) {
	cast_expr *cstexpr = *expr;
	std::vector<token> simple_types;
	terminator_t terminator2;
	size_t i;
	token tok;
	...
	if (peek_type_specifier(simple_types)) {
		...
		tok = lex->get_next_token();
		log_error_at(xlang::filename, tok.loc, "simple type or record name for casting ");
		terminator2.clear();
		terminator2.push_back(PARENTH_CLOSE);
		terminator2.push_back(SEMICOLON);
		terminator2.push_back(COMMA_OP);
...
*/
xlang::assgn_expr *xlang::parser::assignment_expression(terminator_t &terminator, bool is_left_side_handled) {
	assgn_expr *assexpr = nullptr;
	id_expr *idexprtree = nullptr;
	expr *_expr = nullptr;
	id_expr *ptr_ind = nullptr;
	...
	if (expect_assignment_operator()) {
		...
		tok = lex->get_next_token();
		log_error_at(xlang::filename, tok.loc, " assignment operator expected but found ", tok.string);
	}
	return nullptr;
}
...
*/
xlang::func_call_expr *xlang::parser::func_call_expression(terminator_t &terminator) {
	func_call_expr *funccallexp = nullptr;
	...
	if (peek_token(PARENTH_CLOSE)) {
		...
		if (peek_token(terminator)) {
			...
			tok = lex->get_next_token();
			log_error_at(xlang::filename, tok.loc, get_terminator_string(terminator) + " expected in function call but found " + tok.string);
		...
		if (is_expr_terminator_consumed) {
			if (consumed_terminator.number == PARENTH_CLOSE) {
				...
				tok = lex->get_next_token();
				log_error_at(xlang::filename, tok.loc,
				             get_terminator_string(terminator)
				             + " expected in function call but found " + tok.string);
...
*/
void xlang::parser::func_call_expression_list(std::list<expr *> &exprlist, terminator_t &orig_terminator) {
	expr *_expr = nullptr;
	token tok;
		...
		if (is_expr_terminator_consumed) {
			...
			if (is_expr_terminator_consumed) {
				if (consumed_terminator.number == PARENTH_CLOSE) {
					...
					tok = lex->get_next_token();
					log_error_at(xlang::filename, tok.loc, "invalid token found in function call parameters " + tok.string);
...
expression :
  ...
  function-call-expression
*/
xlang::expr *xlang::parser::expression(terminator_t &terminator) {
	token tok, tok2;
	std::vector<token> specifier;
	std::list<token>::iterator lst_it;
	terminator_t terminator2;
	xlang::sizeof_expr *sizeofexpr = nullptr;
	xlang::cast_expr *castexpr = nullptr;
	primary_expr *pexpr = nullptr;
	id_expr *idexpr = nullptr;
	assgn_expr *assgnexpr = nullptr;
	func_call_expr *funcclexpr = nullptr;
	expr *_expr = xlang::tree::get_expr_mem();
	...
	switch (tok.number) {
		...
		case BIT_COMPL :
			...
			if (pexpr == nullptr) {
				log_error(xlang::filename, "error to parse primary expression");
		...
		case LIT_STRING :
			...
			_expr->primary_expression = pexpr;
			if (!peek_token(terminator)) {
				log_error_at(xlang::filename, tok.loc, "semicolon expected " + tok.string);
				xlang::tree::delete_expr(&_expr);
				return nullptr;
			}
			expr_list.clear();
		...
		case IDENTIFIER :
			...
			if (peek_token(DOT_OP) || peek_token(ARROW_OP)
				...
				if (peek_assignment_operator()) {
					...
					idexpr = get_id_expr_tree();
					if (idexpr == nullptr) {
						log_error(xlang::filename, "error to parse id expression");
						xlang::tree::delete_expr(&_expr);
		...
		case PARENTH_OPEN :
			...
			if (type_specifier(tok2.number) || xlang::symtable::search_record(xlang::record_table, tok2.string)) {
				lex->unget_token(tok);
				lex->unget_token(tok2);
				castexpr = cast_expression(terminator);
				if (castexpr == nullptr) {
					log_error(xlang::filename, "error to parse cast expression");
					xlang::tree::delete_expr(&_expr);
		...
		case ARTHM_MUL :
			...
			if (peek_assignment_operator()) {
				assgnexpr = assignment_expression(terminator, false);
				if (assgnexpr == nullptr) {
					log_error(xlang::filename, "error to parse assignment expression");
					xlang::tree::delete_expr(&_expr);
		...
		case INCR_OP :
			...
			if (idexpr == nullptr) {
				log_error(xlang::filename, "error to parse increment expression");
				xlang::tree::delete_expr(&_expr);
			...
			if (peek_assignment_operator()) {
				...
				if (assgnexpr == nullptr) {
					log_error(xlang::filename, "error to parse passignment expression");
					xlang::tree::delete_expr(&_expr);
		...
		case DECR_OP :
			...
			if (idexpr == nullptr) {
				log_error(xlang::filename, "error to parse decrement expression");
				xlang::tree::delete_expr(&_expr);
		...
		case BIT_AND :
			...
			if (idexpr == nullptr) {
				log_error(xlang::filename, "error to parse addressof expression");
				xlang::tree::delete_expr(&_expr);
		...
		case KEY_SIZEOF :
			...
			if (sizeofexpr == nullptr) {
				log_error(xlang::filename, "error to parse sizeof expression");
				xlang::tree::delete_expr(&_expr);
		...
		default :
			log_error_at(xlang::filename, tok.loc, "invalid token found in expression ", tok.string);
			consume_next_token();
...
void xlang::parser::record_specifier() {
	...
	if (record_head(&tok, &isglob, &isextrn)) {
		...
		if (xlang::symtable::search_record(record_table, tok.string)) {
			log_error_at(xlang::filename, tok.loc, "record " + tok.string + " already exists");

			return;
		}
		//otherwise insert record into record table
		xlang::symtable::insert_record(&record_table, tok.string);
		...
		log_error(xlang::filename, "invalid record definition");
	...
	while ((tok = lex->get_next_token()).number != END_OF_FILE) {
		...
		if (peek_type_specifier() || peek_token(IDENTIFIER)) {
			...
			if (types.size() == 1 && types[0].number == IDENTIFIER) {
				if (xlang::symtable::search_record(xlang::record_table, types[0].string)) {
					typeinf->type = RECORD_TYPE;
					typeinf->type_specifier.record_type = types[0];
					typeinf->type_specifier.simple_type.clear();
				}
				else {
					log_error_at(xlang::filename, types[0].loc, "record '" + types[0].string + "' does not exists");
...
void xlang::parser::rec_id_list(struct st_record_node **rec, struct st_type_info **typeinf) {
	...
	if (peek_token(IDENTIFIER)) {
		...
		tok = lex->get_next_token();
		if (xlang::symtable::search_symbol((*rec)->symtab, tok.string)) {
			log_error_at(xlang::filename, tok.loc, "redeclaration of " + tok.string);
			return;
		}
		else {
			xlang::symtable::insert_symbol(&symt, tok.string);
			assert(xlang::last_symbol != nullptr);
			xlang::last_symbol->type_info = *typeinf;
			xlang::last_symbol->symbol = tok.string;
		...
		tok = lex->get_next_token();
		log_error_at(xlang::filename, tok.loc, "identifier expected in record member definition but found " + tok.string);
...
void xlang::parser::rec_subscript_member(std::list<token> &sublst) {
	...
	if (peek_constant_expression()) {
		...
		tok = lex->get_next_token();
		log_error_at(xlang::filename, tok.loc,
		             "constant expression expected but found " + tok.string);
...
void xlang::parser::rec_func_pointer_member(struct st_record_node **rec,
	...
	if (peek_token(IDENTIFIER)) {
		...
		tok = lex->get_next_token();
		if (xlang::symtable::search_symbol((*rec)->symtab, tok.string)) {
			log_error_at(xlang::filename, tok.loc, "redeclaration of func pointer " + tok.string);
			return;
		}
		else {
			xlang::symtable::insert_symbol(&symt, tok.string);
		...
		tok = lex->get_next_token();
		log_error_at(xlang::filename, tok.loc, "identifier expected in record func pointer member definition");
		return;
	}
}
...
void xlang::parser::rec_func_pointer_params(struct st_symbol_info **stinf) {
	...
	if (peek_type_specifier()) {
		...
		tok = lex->get_next_token();
		log_error_at(xlang::filename, tok.loc, "type specifier expected in record func ptr member definition but found " + tok.string);
...
void xlang::parser::simple_declarator_list(struct st_node **st, struct st_type_info **stinf) {
	...
	if (peek_token(IDENTIFIER)) {
		...
		tok = lex->get_next_token();
		if (xlang::symtable::search_symbol((*st), tok.string)) {
			log_error_at(xlang::filename, tok.loc, "redeclaration/conflicting types of " + tok.string);
			return;
		}
		else {
			xlang::symtable::insert_symbol(&(*st), tok.string);
			if (xlang::last_symbol == nullptr)
				return;
			xlang::last_symbol->symbol = tok.string;
			xlang::last_symbol->tok = tok;
		...
		if (peek_token(IDENTIFIER)) {
			...
			tok = lex->get_next_token();
			log_error_at(xlang::filename, tok.loc, "identifier expected in declaration");
			return;
		}
	}
	else {
		tok = lex->get_next_token();
		log_error_at(xlang::filename, tok.loc, "identifier expected in declaration but found " + tok.string);
		tok = lex->get_next_token();
		return;
	}
}
...
void xlang::parser::subscript_declarator(struct st_symbol_info **stsinf) {
	...
	if (peek_constant_expression()) {
		...
		tok = lex->get_next_token();
		log_error_at(xlang::filename, tok.loc, "constant expression expected but found " + tok.string);
	}
	expect_token(SQUARE_CLOSE_BRACKET, true);
	if (peek_token(SQUARE_OPEN_BRACKET)) {
...
void xlang::parser::subscript_initializer(std::vector<std::vector<token>> &arrinit) {
	...
	if (peek_token(LIT_STRING)) {
		...
		if (peek_literal_with_string()) {
			...
			tok = lex->get_next_token();
			log_error_at(xlang::filename, tok.loc, "literal expected in array initializer but found " + tok.string);
...
void xlang::parser::func_params(std::list<struct st_func_param_info *> &fparams) {
	...
	if (peek_type_specifier()) {
		...
		tok = lex->get_next_token();
		log_error_at(xlang::filename, tok.loc, "type specifier expected in function declaration parameters but found " + tok.string);
		return;
	}
}
...
labled-statement :
  identifier :
*/
xlang::labled_stmt *xlang::parser::labled_statement() {
	labled_stmt *labstmt = xlang::tree::get_label_stmt_mem();
	token tok;
	expect_token(IDENTIFIER, false);
	tok = lex->get_next_token();
	labstmt->label = tok;
...
expression-statement :
  expression
*/
xlang::expr_stmt *xlang::parser::expression_statement() {
	expr_stmt *expstmt = xlang::tree::get_expr_stmt_mem();
...
*/
xlang::select_stmt *xlang::parser::selection_statement(st_node **symtab) {
	token tok;
	terminator_t terminator = {PARENTH_CLOSE};
	select_stmt *selstmt = xlang::tree::get_select_stmt_mem();
...
*/
xlang::iter_stmt *xlang::parser::iteration_statement(st_node **symtab) {
	token tok;
	terminator_t terminator = {PARENTH_CLOSE};
	...
	if (peek_token(KEY_WHILE)) {
		...
		if (peek_token(SEMICOLON)) {
			...
			tok = lex->get_next_token();
			log_error_at(xlang::filename, tok.loc, "expression or ; expected in for()");
		}
...
jump-statement :
  ...
  goto identifier
*/
xlang::jump_stmt *xlang::parser::jump_statement() {
	token tok;
	terminator_t terminator = {SEMICOLON};
	jump_stmt *jmpstmt = xlang::tree::get_jump_stmt_mem();
...
*/
xlang::asm_stmt *xlang::parser::asm_statement() {
	...
	if (peek_token(CURLY_CLOSE_BRACKET)) {
		...
		tok = lex->get_next_token();
		log_error_at(xlang::filename, tok.loc, ", or } expected before \"" + tok.string + "\" in asm statement ");
	}
	return asmhead;
}
...
*/
void xlang::parser::asm_statement_sequence(asm_stmt **asmhead) {
	token tok;
	asm_stmt *asmstmt = xlang::tree::get_asm_stmt_mem();
	...
	if (peek_token(SQUARE_OPEN_BRACKET)) {
		...
		if (peek_token(COLON_OP)) {
			...
			tok = lex->get_next_token();
			log_error_at(xlang::filename, tok.loc, "output operand expected " + tok.string);
			return;
		}
		...
		if (peek_token(SQUARE_CLOSE_BRACKET)) {
			...
			tok = lex->get_next_token();
			log_error_at(xlang::filename, tok.loc, "input operand expected " + tok.string);
			return;
		}
...
*/
void xlang::parser::asm_operand(std::vector<st_asm_operand *> &operand) {
	token tok;
	terminator_t terminator = {PARENTH_CLOSE};
	st_asm_operand * asmoprd = xlang::tree::get_asm_operand_mem();
	expect_token(LIT_STRING, false);
	tok = lex->get_next_token();
	...
	if (peek_expression_token()) {
		...
		tok = lex->get_next_token();
		log_error_at(xlang::filename, tok.loc, " expression expected " + tok.string);
		return;
	}
}
...
*/
xlang::stmt *xlang::parser::statement(st_node **symtab) {
	token tok;
	std::vector<token> types;
	...
	while ((tok = lex->get_next_token()).number != END_OF_FILE) {
		...
		if (type_specifier(tok.number)) {
		...
		else if (tok.number == KEY_BREAK || tok.number == KEY_CONTINUE
		         || tok.number == KEY_RETURN || tok.number == KEY_GOTO) {
			lex->unget_token(tok);
			statement = xlang::tree::get_stmt_mem();
			...
			log_error_at(xlang::filename, tok.loc, "invalid token in statement " + tok.string);
			return nullptr;
		}
	}
	return stmthead;
...
void xlang::parser::get_func_info(st_func_info **func_info, token tok, int type, std::vector<token> &types, bool is_extern, bool is_glob) {
...
xlang::tree_node *xlang::parser::parse() {
	token tok[5];
	std::vector<token> types;
	...
	while ((tok[0] = lex->get_next_token()).number != END_OF_FILE) {
		//if global
		if (tok[0].number == KEY_GLOBAL) {
			...
			if (tok[1].number == KEY_RECORD) {
				...
				if (tok[2].number == IDENTIFIER) {
					...
					if (tok[3].number == PARENTH_OPEN) {
						...
						funcit = func_table.find(tok[2].string);
						if (funcit == func_table.end()) {
							func_table.insert(std::pair<std::string, st_func_info *>(tok[2].string, funcinfo));
							expect_token(CURLY_OPEN_BRACKET, true);
							...
							log_error_at(xlang::filename, tok[2].loc, "redeclaration of function " + tok[2].string);
					...
					if (peek_token(PARENTH_OPEN)) {
						xlang::symtable::remove_symbol(&global_symtab, funcname.string);
						...
						funcit = func_table.find(funcname.string);
						if (funcit == func_table.end()) {
							func_table.insert(std::pair<std::string, st_func_info *>(funcname.string, funcinfo));
							expect_token(CURLY_OPEN_BRACKET, true);
							_tree = xlang::tree::get_tree_node_mem();
							_tree->symtab = symtab;
							...
							log_error_at(xlang::filename, funcname.loc, "redeclaration of function " + funcname.string);
							xlang::symtable::delete_func_info(&funcinfo);
							return tree_head;
						}
					}
				...
				if (tok[2].number == IDENTIFIER) {
					...
					if (peek_token(PARENTH_OPEN)) {
						...
						funcit = func_table.find(funcname.string);
						if (funcit == func_table.end()) {
							func_table.insert(std::pair<std::string, struct st_func_info *>(funcname.string, funcinfo));
							expect_token(CURLY_OPEN_BRACKET, true);
							_tree = xlang::tree::get_tree_node_mem();
							_tree->symtab = symtab;
			...
			if (tok[1].number == KEY_RECORD) {
				...
				if (tok[2].number == IDENTIFIER) {
					...
					if (tok[3].number == PARENTH_OPEN) {
						...
						if (funcit == func_table.end()) {
							expect_token(SEMICOLON, true);
							func_table.insert(std::pair<std::string, struct st_func_info *>(tok[2].string, funcinfo));
				...
				if (tok[2].number == IDENTIFIER) {
					...
					if (tok[3].number == PARENTH_OPEN) {
						...
						if (funcit == func_table.end()) {
							...
							log_error_at(xlang::filename, tok[2].loc,
							             "redeclaration of function " + tok[2].string);
							xlang::symtable::delete_func_info(&funcinfo);
			...
			if (tok[1].number == IDENTIFIER) {
				...
				if (tok[2].number == PARENTH_OPEN) {
					...
					funcit = func_table.find(tok[1].string);
					if (funcit == func_table.end()) {
						func_table.insert(std::pair<std::string, struct st_func_info *>(tok[1].string, funcinfo));
						expect_token(CURLY_OPEN_BRACKET, true);
						_tree = xlang::tree::get_tree_node_mem();
						_tree->symtab = symtab;
						...
						log_error_at(xlang::filename, tok[1].loc,
						             "redeclaration of function " + tok[1].string);
				...
				if (peek_token(PARENTH_OPEN) && funcname.number != NONE) {
					...
					if (funcit == func_table.end()) {
						...
						log_error_at(xlang::filename, funcname.loc,
						             "redeclaration of function " + funcname.string);
						xlang::symtable::delete_func_info(&funcinfo);
			...
			if (tok[1].number == IDENTIFIER) {
				...
				if (!xlang::symtable::search_record(xlang::record_table, tok[0].string)) {
					lex->unget_token(tok[1]);
					lex->unget_token(tok[0]);
					_tree = xlang::tree::get_tree_node_mem();
			...
			else if (binary_operator(tok[1].number) || tok[1].number == INCR_OP
			         || tok[1].number == DECR_OP) {
				lex->unget_token(tok[1]);
				lex->unget_token(tok[0]);
				_tree = xlang::tree::get_tree_node_mem();
				_tree->statement = xlang::tree::get_stmt_mem();
				...
				log_error_at(xlang::filename, tok[1].loc,
				             "invalid token found while parsing '" + tok[1].string + "'");
			...
			log_error_at(xlang::filename, tok[0].loc,
			             "invalid token found while parsing '" + tok[0].string + "'");
