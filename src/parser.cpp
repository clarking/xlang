...
namespace xlang {
	...
	st_node *global_symtab;
	//global record table
	st_record_symtab *record_table;
	//function declaration table
	std::map<std::string, st_func_info *> func_table;
	...
	parser::parser() {
		...
		global_symtab = symtable::get_node_mem();
		record_table = symtable::get_record_symtab_mem();
...
//peek token with variable number of provided tokens
	...
	bool parser::peek_expr_literal_token() {
		token tok = lex->get_next_token();
	...
	bool parser::expect_token(token_t tk) {
		token tok = lex->get_next_token();
		if (tok.number != tk) {
			...
			if (find_it != token_lexeme_table.end()) {
				...
				if (!expr_list.empty())
					loc = (*it).loc;
				log_error_at(filename, loc, "expected ", find_it->second);
	...
	bool parser::expect_token(token_t tk, bool consume_token) {
		token tok = lex->get_next_token();
		...
		if (tok.number != tk) {
			...
			if (find_it != token_lexeme_table.end()) {
				...
				log_error_at(filename, loc, "expected ", find_it->second, " but found " + s_quotestring(tok.string));
				std::cout << expr_list << std::endl;
				return false;
			}
		}
	...
	bool parser::expect_token(token_t tk, bool consume_token, std::string str) {
		token tok = lex->get_next_token();
		if (tok.number != tk) {
			log_error_at(filename, tok.loc, "expected ", str);
			std::cout << expr_list << std::endl;
			return false;
		}
		if (!consume_token)
	...
	bool parser::expect_token(token_t tk, bool consume_token, std::string str, std::string arg) {
		token tok = lex->get_next_token();

		if (tok.number != tk) {
			log_error_at(filename, tok.loc, "expected ", str, arg);
			std::cout << expr_list << std::endl;
			return false;
		}
		if (!consume_token)
	...
	bool parser::expect_token(const char *format...) {
		va_list args;
		va_start(args, format);
		token tok = lex->get_next_token();
		...
		va_end(args);
		log_error_at(filename, tok.loc, "expected ");
		return false;
	}
	...
	void parser::consume_n_tokens(int n) {
		while (n > 0) {
			lex->get_next_token();
			n--;
		}
	...
	void parser::consume_tokens_till(terminator_t &terminator) {
		token tok;
	...
	bool parser::match_with_terminator(terminator_t &tkv, token_t tk) {
		for (auto x: tkv) {
			if (x == tk)
				return true;
		}
	...
	std::string parser::get_terminator_string(terminator_t &terminator) {
		std::string st;
		size_t i;
	...
	*/
	...
	bool parser::peek_literal_with_string() {
		token_t tk = get_peek_token();
		return (tk == LIT_DECIMAL || tk == LIT_OCTAL
		        || tk == LIT_HEX || tk == LIT_BIN
		        || tk == LIT_FLOAT || tk == LIT_CHAR
	...
	bool parser::constant_expression(token_t tk) {
		return (integer_literal(tk) || character_literal(tk));
	}

	bool parser::peek_constant_expression() {
	...
	bool parser::expect_binary_opertr_token() {
	...
	bool parser::peek_expression_token() {
	...
	*/
	void parser::primary_expression(terminator_t &terminator) {
		...
		switch (tok.number) {
			case PARENTH_OPEN : {
				...
				if (peek_token(PARENTH_CLOSE)) {
					...
					log_error_at(filename, tok2.loc, "expression expected ", tok2.string);
					return;
				}
				...
				if (parenth_stack.size() > 0 && expect_token(PARENTH_CLOSE)) {
					if (!check_parenth())
						log_error(filename, "unbalanced parenthesis");
					else {
						token tok2 = lex->get_next_token();
					...
					if (peek_binary_operator() || peek_unary_operator()) {
						...
						token tok2 = lex->get_next_token();
						if (!check_parenth())
							log_error_at(filename, tok2.loc, "unbalanced parenthesis ", tok2.string);
						else {
							expr_list.push_back(tok2);
							primary_expression(terminator);
						}
						...
						if (!is_expr_terminator_consumed || !is_expr_terminator_got)
							log_error_at(filename, tok.loc, get_terminator_string(terminator) + "expected");
						if (check_parenth())
							log_error(filename, "unbalanced parenthesis");
						else {
							if (tok.number == END_OF_FILE)
								return;
							log_error(filename, get_terminator_string(terminator) + "expected but found " + tok.string);
						}
					}
				}
			}
			...
			case PARENTH_CLOSE : {
				if (!check_parenth()) {
					...
					if (peek_binary_operator())
						...
						log_error(filename, get_terminator_string(terminator) + "expected ");
						std::cout << expr_list << std::endl;
						return;
					}
				}
			...
			case LIT_CHAR : {
				...
				if (peek_binary_operator() || peek_unary_operator()) {
					...
					if (peek_token(PARENTH_OPEN) || peek_token(IDENTIFIER)) {
						...
						token tok2 = lex->get_next_token();
						log_error_at(filename, tok2.loc, "literal or expression expected ", tok2.string);
				...
				if (peek_token(terminator)) {
					...
					if (peek_token(PARENTH_CLOSE)) {
						if (parenth_stack.size() == 0) {
							token tok2 = lex->get_next_token();
							log_error_at(filename, tok2.loc, "error ", tok2.string);
						}
					}
						...
						if (!is_expr_terminator_consumed) {
							log_error_at(filename, tok2.loc, get_terminator_string(terminator) + "expected");
							return;
						}
					}
					else if (peek_expr_literal_token()) {
			...
			case BIT_COMPL : {
				...
				if (unary_operator(tok.number)) {
					...
					if (peek_token(PARENTH_OPEN) || peek_expr_literal_token() || peek_token(IDENTIFIER)) {
						...
						token tok2 = lex->get_next_token();
						log_error_at(filename, tok2.loc, "literal expected ", tok2.string);
						return;
					}
				}
			}
			...
			default : {
				log_error_at(filename, tok.loc, "primaryexpr invalid token ", tok.string);
				return;
			}
				break;
		}
	...
	void parser::sub_primary_expression(terminator_t &terminator) {
		if (expr_list.size() > 0)
			primary_expression(terminator);
	}
	...
	*/
	int parser::operator_precedence(token_t opr) {
		switch (opr) {
			case DOT_OP :
				return 24;
			case ARROW_OP :
			...
			switch ((*expr_it).number) {
				...
				case ADDROF_OP :
					if (post_stack.empty() || post_stack.top().number == PARENTH_OPEN)
						...
						if (!post_stack.empty() && (operator_precedence((*expr_it).number) > operator_precedence(post_stack.top().number))) {
							post_stack.push(*expr_it);
						}
						else {
							while (!post_stack.empty() && (operator_precedence((*expr_it).number) <= operator_precedence(post_stack.top().number))) {
				...
				default :
					log_error_at(filename, (*expr_it).loc, "error in conversion into postfix expression ", (*expr_it).string);
					return;
			}
			expr_it++;
		}
	...
	primary_expr *parser::get_primary_expr_tree() {
...
//increment/decrement/addressof operators
	id_expr *parser::get_id_expr_tree() {
...
*/
	void parser::id_expression(terminator_t &terminator) {
		token tok = lex->get_next_token();
		...
		if (tok.number == IDENTIFIER) {
			...
			if (peek_token(terminator)) {
				...
				log_error_at(filename, tok.loc, st + " expected in id expression but found ", tok.string);
				std::cout << "    " << expr_list << std::endl;
				return;
			}
		}
		else {
			log_error_at(filename, tok.loc, " identifier expected but found ", tok.string);
			std::cout << "    " << expr_list << std::endl;
		}
	}
	...
	void parser::subscript_id_access(terminator_t &terminator) {
		...
		if (expect_token(SQUARE_OPEN_BRACKET)) {
			...
			if (peek_constant_expression() || peek_identifier()) {
				...
				token tok2 = lex->get_next_token();
				log_error(filename, "constant expression expected ", tok2.string);
				std::cout << expr_list << std::endl;
	...
	*/
	void parser::pointer_indirection_access(terminator_t &terminator) {
		...
		if (peek_token(IDENTIFIER))
			...
			log_error(filename, "identifier expected in pointer indirection");
			std::cout << expr_list << std::endl;
	...
	*/
	id_expr *parser::prefix_incr_expression(terminator_t &terminator) {
		...
		if (peek_token(IDENTIFIER)) {
			...
			log_error(filename, "identifier expected ");
			std::cout << expr_list << std::endl;
	...
	*/
	id_expr *parser::prefix_decr_expression(terminator_t &terminator) {
		id_expr *pridexpr = nullptr;
	...
	*/
	void parser::postfix_incr_expression(terminator_t &terminator) {
		token tok;
		if (expect_token(INCR_OP)) {
			token tok = lex->get_next_token();
			expr_list.push_back(tok);
		...
		if (peek_token(terminator)) {
			...
			tok = lex->get_next_token();
			log_error_at(filename, tok.loc, "; , ) expected but found " + tok.string);
			std::cout << expr_list << std::endl;
	...
	*/
	void parser::postfix_decr_expression(terminator_t &terminator) {
		if (expect_token(DECR_OP)) {
			token tok = lex->get_next_token();
			expr_list.push_back(tok);
		}
	...
	*/
	id_expr *parser::address_of_expression(terminator_t &terminator) {
	...
	sizeof-expression :
	  ...
	  sizeof ( identifier )
	*/
	sizeof_expr *parser::sizeof_expression(terminator_t &terminator) {
		sizeof_expr *sizeofexpr = tree::get_sizeof_expr_mem();
		...
		if (peek_type_specifier(simple_types)) {
			...
			log_error(filename, "simple types, class names or identifier expected for sizeof ");
			terminator2.clear();
			terminator2.push_back(PARENTH_CLOSE);
	...
	*/
	cast_expr *parser::cast_expression(terminator_t &terminator) {
		cast_expr *cstexpr = tree::get_cast_expr_mem();
		token tok;
		expect_token(PARENTH_OPEN, true);
		...
		if (peek_token(IDENTIFIER)) {
			...
			tok = lex->get_next_token();
			log_error_at(filename, tok.loc, " identifier expected in cast expression");
		}
		delete cstexpr;
		return nullptr;
	}

	void parser::cast_type_specifier(cast_expr **expr) {
		...
		if (peek_type_specifier(simple_types)) {
			...
			tok = lex->get_next_token();
			log_error_at(filename, tok.loc, "simple type or record name for casting ");
			terminator2.clear();
			terminator2.push_back(PARENTH_CLOSE);
			terminator2.push_back(SEMICOLON);
			terminator2.push_back(COMMA_OP);
	...
	*/
	assgn_expr *parser::assignment_expression(terminator_t &terminator, bool is_left_side_handled) {
		...
		if (expect_assignment_operator()) {
			...
			tok = lex->get_next_token();
			log_error_at(filename, tok.loc, " assignment operator expected but found ", tok.string);
		}
		return nullptr;
	}
	...
	*/
	func_call_expr *parser::func_call_expression(terminator_t &terminator) {
		...
		if (peek_token(PARENTH_CLOSE)) {
			...
			if (peek_token(terminator)) {
				...
				tok = lex->get_next_token();
				log_error_at(filename, tok.loc, get_terminator_string(terminator) + " expected in function call but found " + tok.string);
			}
		}
		else {
			is_expr_terminator_consumed = false;
	...
	*/
	void parser::func_call_expression_list(std::list<expr *> &exprlist, terminator_t &orig_terminator) {
		expr *_expr = nullptr;
		token tok;
			...
			if (is_expr_terminator_consumed) {
				...
				if (is_expr_terminator_consumed) {
					if (consumed_terminator.number == PARENTH_CLOSE) {
						...
						tok = lex->get_next_token();
						log_error_at(filename, tok.loc, "invalid token found in function call parameters " + tok.string);
					}
				}
				else {
					tok = lex->get_next_token();
	...
	*/
	expr *parser::expression(terminator_t &terminator) {
		...
		terminator_t terminator2;
		sizeof_expr *sizeofexpr = nullptr;
		cast_expr *castexpr = nullptr;
		...
		switch (tok.number) {
			...
			case BIT_COMPL :
				...
				if (pexpr == nullptr) {
					log_error(filename, "error to parse primary expression");
			...
			case LIT_STRING :
				...
				_expr->primary_expression = pexpr;
				if (!peek_token(terminator)) {
					log_error_at(filename, tok.loc, "semicolon expected " + tok.string);
					tree::delete_expr(&_expr);
					return nullptr;
				}
				expr_list.clear();
				...
				if (peek_token(DOT_OP) || peek_token(ARROW_OP)
					...
					if (peek_assignment_operator()) {
						...
						if (assgnexpr == nullptr) {
							log_error(filename, "error to parse assignment expression");
							tree::delete_expr(&_expr);
							return nullptr;
						}
						...
						idexpr = get_id_expr_tree();
						if (idexpr == nullptr) {
							log_error(filename, "error to parse id expression");
							tree::delete_expr(&_expr);
							return nullptr;
						}
						...
						if (funcclexpr == nullptr) {
							log_error(filename, "error to parse function call expression");
							tree::delete_expr(&_expr);
							return nullptr;
						}
			...
			case PARENTH_OPEN :
				...
				if (type_specifier(tok2.number) || symtable::search_record(record_table, tok2.string)) {
					lex->unget_token(tok);
					lex->unget_token(tok2);
					castexpr = cast_expression(terminator);
					if (castexpr == nullptr) {
						log_error(filename, "error to parse cast expression");
						tree::delete_expr(&_expr);
						return nullptr;
					}
			...
			case ARTHM_MUL :
				...
				if (peek_assignment_operator()) {
					...
					idexpr = get_id_expr_tree();
					if (idexpr == nullptr) {
						log_error(filename, "error to parse pointer indirection expression");
						tree::delete_expr(&_expr);
						return nullptr;
					}
					idexpr->is_ptr = true;
			...
			case INCR_OP :
				...
				if (idexpr == nullptr) {
					log_error(filename, "error to parse increment expression");
					tree::delete_expr(&_expr);
					return nullptr;
				}
				//peek for assignment operator
				if (peek_assignment_operator()) {
					...
					if (assgnexpr == nullptr) {
						log_error(filename, "error to parse passignment expression");
						tree::delete_expr(&_expr);
						return nullptr;
					}
					_expr->expr_kind = ASSGN_EXPR;
			...
			case DECR_OP :
				...
				if (idexpr == nullptr) {
					log_error(filename, "error to parse decrement expression");
					tree::delete_expr(&_expr);
			...
			case BIT_AND :
				...
				if (idexpr == nullptr) {
					log_error(filename, "error to parse addressof expression");
					tree::delete_expr(&_expr);
					return nullptr;
				}
				_expr->expr_kind = ID_EXPR;
			...
			case KEY_SIZEOF :
				...
				if (sizeofexpr == nullptr) {
					log_error(filename, "error to parse sizeof expression");
					tree::delete_expr(&_expr);
					return nullptr;
				}
			...
			case SEMICOLON :
				...
				return nullptr;
			default :
				log_error_at(filename, tok.loc, "invalid token found in expression ", tok.string);
				consume_next_token();
				return nullptr;
		}
		return _expr;
	...
	*/
	void parser::record_specifier() {
		...
		if (record_head(&tok, &isglob, &isextrn)) {
			...
			if (symtable::search_record(record_table, tok.string)) {
				log_error_at(filename, tok.loc, "record " + tok.string + " already exists");
				return;
			}
			//otherwise insert record into record table
			symtable::insert_record(&record_table, tok.string);
			rec = last_rec_node;
			...
			log_error(filename, "invalid record definition");
	}
		...
		while ((tok = lex->get_next_token()).number != END_OF_FILE) {
			...
			if (peek_type_specifier() || peek_token(IDENTIFIER)) {
				...
				if (types.size() == 1 && types[0].number == IDENTIFIER) {
					if (symtable::search_record(record_table, types[0].string)) {
						typeinf->type = RECORD_TYPE;
						typeinf->type_specifier.record_type = types[0];
						typeinf->type_specifier.simple_type.clear();
					}
					else
						log_error_at(filename, types[0].loc, "record '" + types[0].string + "' does not exists");
				}
	...
	void parser::rec_id_list(st_record_node **rec, st_type_info **typeinf) {
		...
		if (peek_token(IDENTIFIER)) {
			...
			if (symtable::search_symbol((*rec)->symtab, tok.string)) {
				log_error_at(filename, tok.loc, "redeclaration of " + tok.string);
				return;
			}
			else {
			...
			tok = lex->get_next_token();
			log_error_at(filename, tok.loc, "identifier expected in record member definition but found " + tok.string);
	...
	*/
	void parser::rec_subscript_member(std::list<token> &sublst) {
		...
		if (peek_constant_expression()) {
			...
			tok = lex->get_next_token();
			log_error_at(filename, tok.loc, "constant expression expected but found " + tok.string);
		}
	...
	*/
	void parser::rec_func_pointer_member(st_record_node **rec, int *ptrseq, st_type_info **typeinf) {
		...
		if (peek_token(IDENTIFIER)) {
			...
			tok = lex->get_next_token();
			if (symtable::search_symbol((*rec)->symtab, tok.string)) {
				log_error_at(filename, tok.loc, "redeclaration of func pointer " + tok.string);
				return;
			}
			else {
				symtable::insert_symbol(&symt, tok.string);
			...
			tok = lex->get_next_token();
			log_error_at(filename, tok.loc, "identifier expected in record func pointer member definition");
			return;
		}
	}
	...
	*/
	void parser::rec_func_pointer_params(st_symbol_info **stinf) {
		...
		if (peek_type_specifier()) {
			...
			tok = lex->get_next_token();
			log_error_at(filename, tok.loc, "type specifier expected in record func ptr member definition but found " + tok.string);
	...
	void parser::simple_declarator_list(st_node **st, st_type_info **stinf) {
		...
		if (peek_token(IDENTIFIER)) {
			...
			if (symtable::search_symbol((*st), tok.string)) {
				log_error_at(filename, tok.loc, "redeclaration/conflicting types of " + tok.string);
				return;
			}
			else {
			...
			if (peek_token(IDENTIFIER)) {
				...
				tok = lex->get_next_token();
				log_error_at(filename, tok.loc, "identifier expected in declaration");
				return;
			}
		}
		else {
			tok = lex->get_next_token();
			log_error_at(filename, tok.loc, "identifier expected in declaration but found " + tok.string);
			tok = lex->get_next_token();
			return;
		}
	}
	...
	void parser::subscript_initializer(std::vector<std::vector<token>> &arrinit) {
		...
		if (peek_token(LIT_STRING)) {
			...
			if (peek_literal_with_string()) {
				...
				tok = lex->get_next_token();
				log_error_at(filename, tok.loc, "literal expected in array initializer but found " + tok.string);
			}
			expect_token(CURLY_CLOSE_BRACKET, true);
			if (peek_token(COMMA_OP)) {
				consume_next_token();
	...
	*/
	void parser::func_params(std::list<st_func_param_info *> &fparams) {
		...
		if (peek_type_specifier()) {
			...
			tok = lex->get_next_token();
			log_error_at(filename, tok.loc, "type specifier expected in function declaration parameters but found " + tok.string);
			return;
		}
	}
	...
	iter_stmt *parser::iteration_statement(st_node **symtab) {
		...
		if (peek_token(KEY_WHILE)) {
			...
			if (peek_token(SEMICOLON))
				...
				tok = lex->get_next_token();
				log_error_at(filename, tok.loc, "expression or ; expected in for()");
			}
	...
	*/
	asm_stmt *parser::asm_statement() {
		...
		if (peek_token(CURLY_CLOSE_BRACKET))
			...
			tok = lex->get_next_token();
			log_error_at(filename, tok.loc, ", or } expected before \"" + tok.string + "\" in asm statement ");
		}
		return asmhead;
	}
	...
	void parser::asm_statement_sequence(asm_stmt **asmhead) {
		...
		if (peek_token(SQUARE_OPEN_BRACKET)) {
			...
			if (peek_token(COLON_OP)) {
				...
				tok = lex->get_next_token();
				log_error_at(filename, tok.loc, "output operand expected " + tok.string);
				return;
			}
			...
			if (peek_token(SQUARE_CLOSE_BRACKET))
				...
				tok = lex->get_next_token();
				log_error_at(filename, tok.loc, "input operand expected " + tok.string);
				return;
			}
	...
	*/
	void parser::asm_operand(std::vector<st_asm_operand *> &operand) {
		...
		if (peek_expression_token()) {
			...
			tok = lex->get_next_token();
			log_error_at(filename, tok.loc, " expression expected " + tok.string);
			return;
		}
	}
	...
	stmt *parser::statement(st_node **symtab) {
		...
		while ((tok = lex->get_next_token()).number != END_OF_FILE) {
			...
			if (type_specifier(tok.number)) {
				...
				log_error_at(filename, tok.loc, "invalid token in statement " + tok.string);
				return nullptr;
			}
		}
		return stmthead;
			...
			if (tok[0].number == KEY_GLOBAL) {
					...
					if (tok[2].number == IDENTIFIER) {
						...
						if (tok[3].number == PARENTH_OPEN) {
							...
							if (funcit == func_table.end()) {
								...
								log_error_at(filename, tok[2].loc, "redeclaration of function " + tok[2].string);
						...
						if (peek_token(PARENTH_OPEN)) {
							symtable::remove_symbol(&global_symtab, funcname.string);
							...
							if (funcit == func_table.end()) {
								...
								log_error_at(filename, funcname.loc, "redeclaration of function " + funcname.string);
								symtable::delete_func_info(&funcinfo);
								return tree_head;
							}
						}
					...
					if (tok[2].number == IDENTIFIER) {
						...
						if (tok[3].number == PARENTH_OPEN) {
							...
							if (funcit == func_table.end()) {
								...
								log_error_at(filename, tok[2].loc,
				...
				if (tok[1].number == IDENTIFIER) {
					...
					if (tok[2].number == PARENTH_OPEN) {
						...
						if (funcit == func_table.end()) {
							func_table.insert(std::pair<std::string, st_func_info *>(tok[1].string, funcinfo));
							expect_token(CURLY_OPEN_BRACKET, true);
							_tree = tree::get_tree_node_mem();
							_tree->symtab = symtab;
							...
							log_error_at(filename, tok[1].loc, "redeclaration of function " + tok[1].string);
				...
				if (tok[1].number == IDENTIFIER) {
					...
					if (tok[2].number == PARENTH_OPEN) {
						...
						if (funcit == func_table.end()) {
							...
							log_error_at(filename, tok[1].loc,
					...
					if (!symtable::search_record(record_table, tok[0].string)) {
						lex->unget_token(tok[1]);
						lex->unget_token(tok[0]);
						_tree = tree::get_tree_node_mem();
					...
					log_error_at(filename, tok[1].loc, "invalid token found while parsing '" + tok[1].string + "'");
					return tree_head;
				}
				...
				log_error_at(filename, tok[0].loc, "invalid token found while parsing '" + tok[0].string + "'");
				return tree_head;
			}
		}
		return tree_head;
