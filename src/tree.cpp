...
namespace xlang {
	...
	sizeof_expr *tree::get_sizeof_expr_mem() {
		sizeof_expr *newexpr = new sizeof_expr();
		return newexpr;
	}

	void tree::delete_sizeof_expr(sizeof_expr **soexpr) {
		if (*soexpr == nullptr)
			return;
		delete *soexpr;
		*soexpr = nullptr;
	...
	cast_expr *tree::get_cast_expr_mem() {
		cast_expr *newexpr = new cast_expr();
		return newexpr;
	}

	void tree::delete_cast_expr(cast_expr **cexpr) {
		if (*cexpr == nullptr)
			return;
		delete *cexpr;
		*cexpr = nullptr;
	...
	primary_expr *tree::get_primary_expr_mem() {
	...
	void tree::get_inorder_primary_expr(primary_expr **pexpr) {
	...
	void tree::delete_primary_expr(primary_expr **pexpr) {
		if (*pexpr == nullptr)
			return;
	...
	void tree::delete_id_expr(id_expr **idexpr) {
		if (*idexpr == nullptr)
			return;
	...
	assgn_expr *tree::get_assgn_expr_mem() {
	...
	void tree::delete_assgn_expr(assgn_expr **exp) {
		if (*exp == nullptr)
			return;
	...
	func_call_expr *tree::get_func_call_expr_mem() {
		func_call_expr *newexpr = new func_call_expr();
		newexpr->function = nullptr;
		return newexpr;
	}

	void tree::delete_func_call_expr(func_call_expr **exp) {
