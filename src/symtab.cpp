...
//memory deallocation functions
	...
	//hashing functions
	unsigned int symtable::st_hash_code(lexeme_t lxt) {
	...
	unsigned int symtable::st_rec_hash_code(lexeme_t lxt) {
		void *key = (void *) lxt.c_str();
		unsigned int murhash = MurmurHash3_x86_32(key, lxt.size(), 4);
	...
	void symtable::insert_symbol(st_node **symtab, lexeme_t symbol) {
	...
	bool symtable::search_symbol(st_node *st, lexeme_t symbol) {
		if (st == nullptr)
			return false;
	...
	st_symbol_info *symtable::search_symbol_node(st_node *st, lexeme_t symbol) {
		if (st == nullptr)
			return nullptr;
		st_symbol_info *temp = nullptr;
		temp = st->symbol_info[st_hash_code(symbol)];
	...
	void symtable::insert_symbol_node(st_node **symtab, st_symbol_info **syminf) {
		...
		temp = symtable::search_symbol_node(*symtab, (*syminf)->symbol);
		if (temp != nullptr)
			temp = *syminf;
	}

	bool symtable::remove_symbol(st_node **symtab, lexeme_t symbol) {
		st_symbol_info *temp = nullptr;
	...
	void symtable::insert_record(st_record_symtab **recsymtab, record_t recordname) {
	...
	bool symtable::search_record(st_record_symtab *rec, record_t recordname) {
		if (rec == nullptr)
			return false;
	...
	st_record_node *symtable::search_record_node(st_record_symtab *rec, record_t recordname) {
		if (rec == nullptr)
			return nullptr;
		st_record_node *temp = nullptr;
		temp = rec->recordinfo[st_rec_hash_code(recordname)];
